...
class PullRequestScraper:
    ...
    def scrape_pull_requests(self, repo_url: str, limit: int = 50) -> List[PullRequestInfo]:
        ...
        try:
            ...
            logger.info(f"Successfully scraped {len(pull_requests)} pull requests for {repo_url}")
    ...
    def _get_pr_urls(self, repo_url: str, limit: int) -> List[str]:
        ...
        try:
            pulls_url = urljoin(repo_url, '/pulls?q=is%3Apr')
            ...
            for link in pr_links[:limit]:
                ...
                if href and '/pull/' in href:
                    full_url = urljoin(repo_url, href)
