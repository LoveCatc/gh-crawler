...
class RepositoryScraper:
    ...
    def _extract_issues_counts(
        ...
        try:
            ...
            open_link = issues_soup.find(
                "a", href=re.compile(r"/issues\?q=is%3Aopen")
            )
            if open_link:
            ...
            closed_link = issues_soup.find(
                "a", href=re.compile(r"/issues\?q=is%3Aclosed")
            )
            if closed_link:
    ...
    def _extract_pr_counts(
        ...
        try:
            ...
            open_link = pulls_soup.find(
                "a", href=re.compile(r"/pulls\?q=is%3Aopen")
            )
            if open_link:
                text = open_link.get_text(strip=True)
                numbers = re.findall(r"[\d,]+", text)
            ...
            closed_link = pulls_soup.find(
                "a", href=re.compile(r"/pulls\?q=is%3Aclosed")
            )
            if closed_link:
                text = closed_link.get_text(strip=True)
                numbers = re.findall(r"[\d,]+", text)
